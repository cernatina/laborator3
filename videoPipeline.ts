...
function downloadYouTubeVideo(videoUrl: string, outputPath: string): Promise<string> {
  return new Promise((resolve, reject) => {
    const command = `"C:\\Users\\Inna\\Downloads\\yt-dlp.exe" -f "bv*[height<=720][ext=mp4]+ba[ext=m4a]/b[height<=720]/b" -o "${outputPath.replace(/\\/g, '\\\\')}.mp4" ${videoUrl}`;
...
async function runProducer(): Promise<void> {
  ...
  let ids = ["it1rTvBcfRg", "Gjnup-PuquQ", "i4ZuseKFBF0"];
  for (let id of ids) {
    try {
